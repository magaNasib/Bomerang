class Wordle {
  constructor() {
    ...
    this.word = this.toLocaleUpperCase(
    ...
    this.keyboard = [
      ["Q", "Ü", "E", "R", "T", "Y", "U", "İ", "O", "P", "Ö", "Ğ", "0"],
      ["0", "A", "S", "D", "F", "G", "H", "J", "K", "L", "I", "Ə", "0"],
      ["Enter", "Z", "X", "C", "V", "B", "N", "M", "Ç", "Ş", "del", "0", "0"],
    ];
    this.letterEnd = [[], [], [], [], [], []];
    ...
    for (let i = 0; i < 6; i++) {
      ...
      for (let j = 0; j < 5; j++) {
        ...
        if (this.letterEnd[i][j] === "green") {
          ...
          if (this.entered[i] === false) e.classList.add("anim");
          else e.classList.remove("anim");
        }
    ...
    for (let i = 0; i < 3; i++) {
      ...
      for (let j = 0; j < 13; j++) {
        if (this.keyboard[i][j] != "0") {
          let e = document.createElement("div");
          e.innerHTML = this.keyboard[i][j];
          e.classList.add("key");

          let _keyBoardMarked = false,
            color;

          for (let k = 0; k < this.keyBoardMarked.length; k++) {
            if (this.keyBoardMarked[k][1] === e.innerHTML) {
              color = this.keyBoardMarked[k][0];
              if (
                this.keyBoardMarked[k][0] != "green" &&
                this.word.indexOf(e.innerHTML) ===
                this.inputWord.indexOf(e.innerHTML) &&
                this.word.indexOf(e.innerHTML) != -1 &&
                this.inputWord.indexOf(e.innerHTML) != -1
              )
                color = "green";
          ...
          if (_keyBoardMarked) {
            ...
            for (let k = 0; k < 5; k++) {
              if (this.inputWord[k] === e.innerHTML) {
                let c;
                if (this.success[k] === "green") {
                  setTimeout(() => {
                    e.classList.add("green");
                  }, 300);
                  c = "green";
                ...
                this.keyBoardMarked.push([c, e.innerHTML]);
              }
            }
          }
          if (this.keyboard[i][j] === "del") {
  ...
  checkLetters() {
    let inputWord = this.grid[this.row].join("");
    inputWord = inputWord.toLocaleUpperCase();

    const words = this.words.map((word) => word.toLocaleUpperCase());
    ...
    if (words.indexOf(inputWord) === -1) {
      ...
      for (let i = 0; i < 5; i++) {
        if (this.letterEnd[this.row][i] != "green") success = false;
      }
